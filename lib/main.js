...
const getVideo = async (csvFile, startIndex = 1) => {
  ...
  await taskQueue.addTasks(words.map(word => async () => {
    ...
    await Promise.all([
      ...
      ]).then(() => generateVideo({ videoPath: wordVideoPath, audioPath: wordAudioPath, imagePath: wordImagePath })),
      Promise.all([
      ...
      ]).then(() => generateVideo({ videoPath: exampleVideoPath, audioPath: exampleAudioPath, imagePath: exampleImagePath }))
    ]);
