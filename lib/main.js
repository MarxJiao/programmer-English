...
const getVideo = async (csvFile) => {
  ...
  for (const word of words) {
    ...
    await generateAudio(`${word.word}。${word.translation}`, wordAudioPath);
    ...
    await generateAudio(`${word.example}。${word.exampleTranslation}`, exampleAudioPath);
    await generateImage(`${word.example}\n${word.exampleTranslation}`, exampleImagePath);
